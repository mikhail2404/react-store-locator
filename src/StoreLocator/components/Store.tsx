...
const Wrapper = styled.div<IIsActive>`
  && {
    cursor: pointer;
    padding: 25px 30px;
...
const Info = styled.div<IIsActive>`
  line-height: 15px;
  display: none;
...
const Website = styled.a<IWebsite>`
  font-size: 12px;
  letter-spacing: 0.5px;
...
const Store = ({ item, mapInstance, style }: IProps) => {
  ...
  const activeClass = "nid" in store && store.nid === nid ? "active" : "";
  const isActive = "nid" in store && store.nid === nid;
