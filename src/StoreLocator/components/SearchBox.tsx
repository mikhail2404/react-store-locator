...
import {IPlace, Styles} from "../../types/interfaces";
...
interface IProps {
  mapApi: any;
  fetchPlaces: (data: IPlace[]) => void;
  fetchInput: (value: string) => void;
  map: any;
  style: Styles;
}

const SearchBox = ({ mapApi, fetchPlaces, fetchInput, map, style }: IProps) => {
  const {
    storeLocatorState,
    setActivePlaces,
    setClosest,
    ...
    <Wrapper className="store-locator-searchBox">
      {!input ? (
        ...
        <ResetIcon
          ...
          <IconArrowLeft style={arrowLeftIconStyles}/>
        </ResetIcon>
      )}
      <Input
        ref={searchInput}
