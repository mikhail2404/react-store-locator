...
const StoreLocatorComponent = ({ styles, storeLabel, googleMapApiKey }: StoreLocatorProps) => {
  ...
  const GOOGLE_MAP_API = googleMapApiKey;
  ...
  const [changeCenter, setChangeCenter] = useState<LatLng | {lat: null, lng: null}>({lat: null, lng: null});
  ...
  const defaultCenter = {lat: defaultSettings.center[0], lng: defaultSettings.center[1]}
  useEffect(() => {
  ...
  useEffect(() => {
    const layout = document.querySelector<HTMLElement>(".layout-content")!;
  ...
  const fetchData = () => {
    fetch("/store-locator-json")
      ...
      .then((data) => {
        const newData = data.map((item: RawStore) => {
          const {
        ...
        const countriesCount = newData.reduce((a: StoreCount, { country } : Country) => {
        ...
        const result = newData.map((item: IStore, index: number, self: IStore[]) => {
          const { country } = item;
          ...
          return {
            ...
            show:
              index === self.findIndex((t: IStore) => t.country === item.country) ||
              false
          };
        });
  ...
  const filterData = (value: string) => {
    const result = allData.filter((data: IStore) => {
  ...
  const fetchPlaces = (data: IPlace[]) => {
    setPlaces(data);
  };

  const fetchInput = (value: string) => {
    setInput(value);
  ...
  const checkCountryCenter = (item: IStore, value = "") => {
    const { country } = item;

    const countriesCoords = allData
      .filter((data: IStore) => data.country === (country || value))
      .map((item: IStore) => {
        return {
          lat: item.lat,
          lng: item.lng
  ...
  const onClick = ({ item, type }: onClickProps) => {
  ...
  const getGeocodeData = (response: Response) => {
  ...
  const onChange = ({ center, bounds }: Location) => {
    ...
    if (allData) {
      const data = allData.filter((marker: IStore) => {
        return (
          marker.lat > se.lat &&
          sw.lat &&
          marker.lat < ne.lat &&
  ...
  const haversineDistance = (latLng: LatLng) => {
  ...
  const scrollIfNeeded = (element: HTMLElement, container: HTMLElement) => {
